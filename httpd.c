...
char end_with(const char* str,const char* sub){
    int i = strlen(str) ;
    int j = strlen(sub);
    while(i&&j){
        i--;
...
void cat(int client, FILE *resource) {
    char buf[1024];
    int len;
    while ((len=fread(buf,1,1024,resource))>0) {
...
void execute_cgi(int client, const char *path, const char *method, const char *query_string) {
    ...
    if (pid == 0) /* child: CGI script */
    {
        ...
        close(cgi_output[1]);
        close(cgi_input[0]);
        if (strcasecmp(method, "POST") == 0) for (i = 0; i < content_length; i++) {
            recv(client, &c, 1, 0);
...
void headers(int client, const char *filename) {
    ...
    const key_value* kv = CONTENT_TYPES;
    while(kv->key){
        if(end_with(filename,kv->key)){
